...
import { useNetworkVariable, networkConfig } from "@/networkConfig";
...
export default function MyAmuletPage() {
  ...
  const currentPackageId = useNetworkVariable("packageId");

  const { data: amuletData, isLoading } = useSuiClientQuery(
    "getOwnedObjects",
    {
      owner: account?.address!,
      filter: { StructType: `${currentPackageId}::miku_cult::DevotionAmulet` },
    ...
    },
    {
      enabled:
        !!account &&
        !!currentPackageId &&
        !currentPackageId.includes("PLACEHOLDER"),
    },
  );
  ...
  const getNetworkName = () => {
    if (currentPackageId === networkConfig.testnet.variables.packageId) {
      return "testnet";
    }
    if (currentPackageId === networkConfig.mainnet.variables.packageId) {
      return "mainnet";
    }
    return "devnet";
  };
  ...
  const explorerUrl = `https://suiscan.xyz/${networkName}/object/${amulet.data.objectId}`;
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col items-center gap-4 text-center animate-fade-in p-4 relative">
      ...
      <div className="mb-4">
        <img
          src={
            display?.image_url?.replace("{rank}", fields.rank) || "/unknown.jpg"
          }
